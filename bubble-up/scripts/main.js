...
'use strict';
{
	...
	const HANDLER_CLASS = class ListDOMHandler extends self.DOMElementHandler {
		constructor(iRuntime) {
			...
			this.AddDOMElementMessageHandler("set-item",
				(elem, e) => this._OnSetItem(elem, e));
	...
	self.C3WebAudioBuffer = class C3WebAudioBuffer extends self.C3AudioBuffer {
		...
		async _Load() {
			try {
                ...
                // new Promise(function (resolve, reject) {
                    ...
                    xhttp.onreadystatechange = async function() {
                        if (this.readyState == 4 && this.status == 200) {
							...
							self._audioData = xhttp.response;
					if (self._audioBuffer) return self._audioBuffer;
					self._audioBuffer =
					await self._audioDomHandler.DecodeAudioData(self._audioData, self._needsSoftwareDecode);
                ...
                // }).then((response) => {
				// 	// console.info(self)
				// 	self._audioData = response;
				// 	if (self._audioBuffer) return self._audioBuffer;
				// 	self._audioBuffer =
				// 	self._audioDomHandler.DecodeAudioData(self._audioData, self._needsSoftwareDecode);
				// 	self._audioData = null
                // })
				// await this._Fetch();
				// await this._Decode();
				this._loadState = "loaded"
